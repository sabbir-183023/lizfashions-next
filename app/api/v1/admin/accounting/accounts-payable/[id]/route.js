// app/api/v1/admin/accounting/accounts-payable/[id]/route.js
...
export async function GET(request, { params }) {
  try {
    ...
    const account = await AccountsPayable.findById(id);
...
export async function PUT(request, { params }) {
  try {
    ...
    const { date, dueDate, supplierName, supplierPhone, supplierAddress, amount, paidAmount, remarks } = body;
    ...
    const updatedAccount = await AccountsPayable.findByIdAndUpdate(
      id,
      {
        date: new Date(date),
...
export async function DELETE(request, { params }) {
  try {
    ...
    const deletedAccount = await AccountsPayable.findByIdAndDelete(id);
