// app/components/admin/accounting/AccountsReceivable.jsx
"use client";
...
const AccountsReceivable = () => {
  ...
  const fetchAccounts = useCallback(async () => {
    setLoading(true);
    try {
      let url = "/api/v1/admin/accounting/accounts-receivable?limit=500";
  ...
  const handleAddAccount = async () => {
    if (!addFormData.payerName || !addFormData.amount || !addFormData.date) {
      toast.error("Payer name, amount, and date are required");
      return;
    }
    ...
    try {
      const response = await fetch(
        "/api/v1/admin/accounting/accounts-receivable",
        {
          method: "POST",
  ...
  const handleEditClick = (account) => {
    ...
    setEditFormData({
      ...
      dueDate: account.dueDate
        ? moment(account.dueDate).format("YYYY-MM-DD")
        : "",
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/accounting/accounts-receivable/${editingAccount._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (data.success) {
        toast.success(
          `Payment of ৳${parseFloat(paymentAmount).toLocaleString()} recorded successfully`,
        );
  ...
  const handleDeleteClick = async (accountId, payerName) => {
    if (
      !confirm(`Are you sure you want to delete account for "${payerName}"?`)
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/accounting/accounts-receivable/${accountId}`,
        {
          method: "DELETE",
        },
      );
        ...
        const updatedFiltered = searchTerm
          ? updatedAccounts.filter(
              (a) =>
                ...
                a.payerPhone?.toLowerCase().includes(searchTerm.toLowerCase()),
            )
          : updatedAccounts;
  ...
  const handlePrint = () => {
    ...
    <html>
      ...
      <body>
          ...
          <tbody>
            ${filteredAccounts
              ...
              <tr>
                ...
                <td>${account.payerName}</td>
                <td>${account.payerPhone || "-"}</td>
        ...
        <div class="footer">
          <p>Generated by Accounts Receivable System</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
              <FaMoneyBillWave /> Accounts Receivable
            </h1>
            <p className="text-blue-200 text-xs sm:text-sm mt-1">
              Track and manage money owed by customers
            </p>
          </div>
      ...
      <div className="p-4 sm:p-6 flex-shrink-0 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by payer name or phone..."
              value={searchTerm}
                  ...
                  >
                    ...
                    <td className="px-3 sm:px-4 py-3 text-center">
                      <div className="flex gap-2 justify-center">
                        ...
                        <button
                          onClick={() =>
                            handleDeleteClick(account._id, account.payerName)
                          }
                          disabled={deletingId === account._id}
            ...
            <div className="p-6">
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    name="payerAddress"
                    value={addFormData.payerAddress}
                    onChange={handleAddInputChange}
                    rows="2"
                    className="w-full px-3 py-2 text-gray-700 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-400"
            ...
            <div className="p-6">
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    name="payerAddress"
                    value={editFormData.payerAddress}
                    onChange={handleEditInputChange}
                    rows="2"
                    className="w-full px-3 py-2 text-gray-700 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-400"
                  />
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            ...
            <div className="p-6 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800">
                    {moment(viewingAccount.date).format("DD-MM-YYYY")}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Due Date</p>
                  <p className="text-sm font-medium text-gray-800">
                    {viewingAccount.dueDate
                      ? moment(viewingAccount.dueDate).format("DD-MM-YYYY")
                      : "-"}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800">
                  {viewingAccount.payerName}
                </p>
              </div>

              {viewingAccount.payerPhone && (
                <div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    ...
                    <p className="text-sm font-bold text-blue-600">
                      ৳{viewingAccount.amount.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Paid</p>
                    <p className="text-sm font-bold text-green-600">
                      ৳{viewingAccount.paidAmount.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Remaining</p>
                    <p className="text-sm font-bold text-red-600">
                      ৳{viewingAccount.remainingAmount.toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <span
                  className={`inline-block px-2 py-1 rounded-full text-xs font-medium mt-1 ${statusColors[viewingAccount.status]}`}
                >
            ...
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 p-3 rounded-lg">
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-semibold">
                    {editingAccount.payerName}
                  </span>
                </p>
                <p className="text-sm text-gray-600">
                  Total Amount:{" "}
                  <span className="font-semibold">
                    ৳{editingAccount.amount.toLocaleString()}
                  </span>
                </p>
                <p className="text-sm text-gray-600">
                  Already Paid:{" "}
                  <span className="font-semibold text-green-600">
                    ৳{editingAccount.paidAmount.toLocaleString()}
                  </span>
                </p>
                <p className="text-sm text-gray-600">
                  Remaining:{" "}
                  <span className="font-semibold text-red-600">
                    ৳{editingAccount.remainingAmount.toLocaleString()}
                  </span>
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Max: ৳{editingAccount.remainingAmount.toLocaleString()}
                </p>
              </div>
